...
export default class GameView{
  ...
  processMassAssign(cellX, cellY){
    ...
    for(let delta = 1; delta <= 3; delta++){
      ...
      deltas.forEach((move) => {
        if(this.pokeBattlefield.inBounds([cellX + move[0], cellY + move[1]])){
          this.pokeBattlefield.assignCell(cellX + move[0], cellY + move[1], this.typeToAssign);
          this.pokeBattlefield.drawCell(battlefield[cellX + move[0]][cellY + move[1]], this.stage, this.squareSize, cellX + move[0], cellY + move[1], this.showGridLines);
        }
      })
    }
