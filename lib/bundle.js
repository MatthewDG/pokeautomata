...
var GameView = function () {
  ...
  _createClass(GameView, [{
    ...
    key: 'processMassAssign',
    value: function processMassAssign(cellX, cellY) {
      var _this = this;

      this.processAssign(cellX, cellY);
      var battlefield = this.pokeBattlefield.grid;
      for (var delta = 1; delta <= 3; delta++) {

        var deltas = [[delta, 0], [delta, delta], [0, delta], [-delta, delta], [-delta, 0], [-delta, -delta], [0, -delta], [delta, -delta]];

        deltas.forEach(function (move) {
          if (_this.pokeBattlefield.inBounds([cellX + move[0], cellY + move[1]])) {
            _this.pokeBattlefield.assignCell(cellX + move[0], cellY + move[1], _this.typeToAssign);
            _this.pokeBattlefield.drawCell(battlefield[cellX + move[0]][cellY + move[1]], _this.stage, _this.squareSize, cellX + move[0], cellY + move[1], _this.showGridLines);
          }
        });
      }
    ...
    key: 'processAssign',
    value: function processAssign(cellX, cellY) {
...
var _jqueryZoom = __webpack_require__(5);
...
var pokeAutomataInterface = function () {
  ...
  _createClass(pokeAutomataInterface, [{
    ...
    key: 'changeAssignInstruction',
    value: function changeAssignInstruction(type) {
      switch (type) {
        case "NORMAL":
...
var _poke_cell = __webpack_require__(4);
...
var Board = function () {
  ...
  _createClass(Board, [{
    ...
    key: "drawCell",
    value: function drawCell(pokeCell, stage, squareSize, idx, idx2, stroke) {
      var cell = new createjs.Shape();

      cell.graphics.beginFill(pokeCell.color);

      if (stroke) {
        cell.graphics.beginStroke('#ddd');
      }

      cell.graphics.drawRect(idx * squareSize, idx2 * squareSize, squareSize, squareSize);
      stage.addChild(cell);
    }
  }, {
    key: "draw",
...
var _game_view = __webpack_require__(0);

var _game_view2 = _interopRequireDefault(_game_view);

var _poke_automata_interface = __webpack_require__(1);
